export default {
  __not: [],
  listNull: {
    zh: '列表为空，请调整过滤条件再试',
    jp: '列表为空，请调整过滤条件再试',
    en: 'Empty. Please change the filter and try again',
  },
  unit: {
    zh: '个插件',
    jp: '个插件',
    en: '',
  },
  plugin: {
    zh: '插件',
    jp: '插件',
    en: 'Plugin',
  },
  numberOfPlugIns: {
    zh: '插件数量',
    jp: '插件数量',
    en: 'Number of plugins',
  },
  numPlugins: {
    zh: '个插件',
    jp: '个插件',
    en: 'Plugins',
  },
  running: {
    zh: '运行中',
    jp: '运行中',
    en: 'Running',
  },
  individual: {
    zh: '个',
    jp: '个',
    en: '',
  },
  stop: {
    zh: '停 止',
    jp: '停 止',
    en: 'Stop',
  },
  stopped: {
    zh: '已停止',
    jp: '已停止',
    en: 'Stopped',
  },
  pluginsList: {
    zh: '插件列表',
    jp: '插件列表',
    en: 'Plugins',
  },
  selectNode: {
    zh: '节点选择',
    jp: '节点选择',
    en: 'Node',
  },
  all: {
    zh: '全部',
    jp: '全部',
    en: 'All',
  },
  startRunning: {
    zh: '启 动',
    jp: '启 动',
    en: 'Run',
  },
  config: {
    zh: '配置',
    jp: '配置',
    en: 'config',
  },
  tutorial: {
    zh: '使用文档',
    jp: '使用文档',
    en: 'Tutorial',
  },
  pluginName: {
    zh: '插件名称',
    jp: '插件名称',
    en: 'Name',
  },
  version: {
    zh: '版本',
    jp: '版本',
    en: 'Version',
  },
  describe: {
    zh: '描述',
    jp: '描述',
    en: 'Description',
  },
  type: {
    zh: '类型',
    jp: '类型',
    en: 'Type',
  },
  state: {
    zh: '状态',
    jp: '状态',
    en: 'State',
  },
  authentication: {
    zh: '认证',
    jp: '认证',
    en: 'Auth',
  },
  backend: {
    zh: '持久化',
    jp: '持久化',
    en: 'Backend',
  },
  bridge: {
    zh: '桥接',
    jp: '桥接',
    en: 'Bridge',
  },
  protocol: {
    zh: '协议',
    jp: '协议',
    en: 'Protocol',
  },
  feature: {
    zh: '功能',
    jp: '功能',
    en: 'Feature',
  },
  other: {
    zh: '其他',
    jp: '其他',
    en: 'Other',
  },
  thisActionWillStopThePlugIn: {
    zh: '此操作将停止该插件,确认继续?',
    jp: '此操作将停止该插件,确认继续?',
    en: 'Confirm to stops the plugin?',
  },
  runSuccess: {
    zh: '开启成功',
    jp: '开启成功',
    en: 'Run Success',
  },
  runFailed: {
    zh: '开启失败',
    jp: '开启失败',
    en: 'Run Failed',
  },
  stopSuccess: {
    zh: '停止成功',
    jp: '停止成功',
    en: 'Stop Success',
  },
  pluginTips: {
    zh: '该功能仅做调试使用，配置将在 Broker 重启后将丢失，请将配置写入相应配置文件',
    jp: '该功能仅做调试使用，配置将在 Broker 重启后将丢失，请将配置写入相应配置文件',
    en: 'The module is only used for debugging or testing, and the configuration will be lost when broker restart',
  },
  configuration: {
    zh: '配置信息',
    jp: '配置信息',
    en: 'Configuration',
  },
  save: {
    zh: '保存',
    jp: '保存',
    en: 'Save',
  },
  updateSuccessful: {
    zh: '更新成功',
    jp: '更新成功',
    en: 'Update Successful',
  },
  pluginBeforeSaveTips: {
    zh: '你的更改将不被保存，确认继续？',
    jp: '你的更改将不被保存，确认继续？',
    en: 'Your changes will not be saved. Confirm to continue?',
  },
  pleaseEnter: {
    zh: '请输入',
    jp: '请输入',
    en: 'Please enter',
  },
  searchByName: {
    zh: '按插件名称搜索',
    jp: '按插件名称搜索',
    en: 'Search by plugin name',
  },
  manage: {
    zh: '管理',
    jp: '管理',
    en: 'Manage',
  },
  authClientidRequired: {
    zh: '请填写 Client ID 和密码',
    jp: '请填写 Client ID 和密码',
    en: 'Client ID or Password is required',
  },
  confirmDelete: {
    zh: '确认删除？',
    jp: '确认删除？',
    en: 'Confirm Delete?',
  },
  algorithm: {
    zh: '加密算法',
    jp: '加密算法',
    en: 'Algorithm',
  },
  payloadDesc: {
    zh: '启用 verify_claims 时有效, 可以使用 %u，%c 占位符分别替换输入的 username 和 clientid，详见',
    jp: '启用 verify_claims 时有效, 可以使用 %u，%c 占位符分别替换输入的 username 和 clientid，详见',
    en: 'It is valid when verify_claims is enabled. You can use% u and% c placeholders to replace the entered user name and clientid respectively. For details, see',
  },
  jwtDoc: {
    zh: 'JWT 认证',
    jp: 'JWT 认证',
    en: 'JWT Authentication',
  },
  dataDesc: {
    zh: '一行一组数据，使用逗号分割 username,clientid',
    jp: '一行一组数据，使用逗号分割 username,clientid',
    en: 'One line is a set of data, separated by comma username,clientid',
  },
  secret: {
    zh: '密钥或私钥',
    jp: '密钥或私钥',
    en: 'Secret or Private key',
  },
  secretRequired: {
    zh: '请输入 Secret',
    jp: '请输入 Secret',
    en: 'Secret is required',
  },
  payloadRequired: {
    zh: '请输入 Payload 模版',
    jp: '请输入 Payload 模版',
    en: 'Payload template is required',
  },
  dataRequired: {
    zh: '请填写 Payload 数据',
    jp: '请填写 Payload 数据',
    en: 'Payload data is required',
  },
  leaveTokenPage: {
    zh: '离开页面后当前输入的配置与生成的 TOKEN 信息均不再保留，确认离开？',
    jp: '离开页面后当前输入的配置与生成的 TOKEN 信息均不再保留，确认离开？',
    en: 'After leaving the page, the currently entered configuration and generated TOKEN information are no longer retained. Continue?',
  },
  usernameOrClientid: {
    zh: '用户名或 Client ID',
    jp: '用户名或 Client ID',
    en: 'Username or Client ID',
  },
  mnesiaTip: {
    zh: '默认使用 Username，以 auth.mnesia.as 配置为准',
    jp: '默认使用 Username，以 auth.mnesia.as 配置为准',
    en: 'Username is used by default, but follow the auth.mnesia.as configuration',
  },
  authMnesiaRequired: {
    zh: '请填写完整的认证信息',
    jp: '请填写完整的认证信息',
    en: 'Authentication information is required',
  },
  auth: {
    zh: '认证',
    jp: '认证',
    en: 'Authentication',
  },
  isAllow: {
    zh: '是否允许',
    jp: '是否允许',
    en: 'Allowed',
  },
  allow: {
    zh: '允许',
    jp: '允许',
    en: 'Allow',
  },
  deny: {
    zh: '不允许',
    jp: '不允许',
    en: 'Deny',
  },
  aclMnesiaRequired: {
    zh: '请填写完整的 ACL 信息',
    jp: '请填写完整的 ACL 信息',
    en: 'ACL information is required',
  },
  action: {
    zh: '主题动作',
    jp: '主题动作',
    en: 'Action',
  },
  allUsers: {
    zh: '全部用户',
    jp: '全部用户',
    en: 'All Users',
  },
}
