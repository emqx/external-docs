export default {
  ...
  deleteResource: {
    ...
    jp: 'この操作を実行するとリソースが削除されます。このまま処理を続けてもよろしいですか?',
  ...
  definingRuleConditionsAndDataProcessing: {
    ...
    jp: 'SQlでルールを設定して、メッセージデータをフィルター・エンコード・デコードと編集が可能です。シームレスに処理後のデータをデーターベース・ストリーム処理・APIゲートウェイに転送できます。',
  ...
  jsonDecodeUpgrade: {
    ...
    jp: 'カレントバージョンでは自動的にpayloadのJSONにデコード機能が廃棄されました。payloadのフィールドを選択して、json_decodeでデコードしてください。例として、',
  ...
  deleteRuleConfirm: {
    ...
    jp: 'この操作を実行するとルールが削除されます。このまま処理を続けてもよろしいですか？',
  ...
  payload_doc: {
    ...
    jp: 'JSONフォーマットの場合、payloadが自動的にデコードされます。 SQL文中にpayload.xでオブジェクト情報を取得することができます',
  },
  peername_doc: {
    en: 'Client peer host',
    zh: '客户端网络地址',
  ...
  parse_confirm: {
    ...
    jp: '互換性がないの古いバージョンのSQL文が見つかりました。新しいバージョンに変更しますか？',
  },
  fallbackAction: {
    en: 'Fallback Actions',
    zh: '失败备选动作',
  ...
  fallbackActionCreate: {
    ...
    jp: 'アクション実行エラーが発生したとき、エラー処理の動作を設定します（任意）',
  },
  fallbackActionTip: {
    en: 'Action to be performed when the action execution error',
    zh: '动作执行错误时将执行的操作',
    jp: 'アクション実行エラーが発生したときの動作',
  },
}
