export default {
  __not: [
    '此操作将停用并删除该资源,确认继续',
    '规则要处理的主题，支持',
    '主题通配符',
    '规则引擎使用说起来语句配置二面消息流与设备事件的处理规则，内置多种灵活的数据处理方案',
  ],
  foldDetails: {
    zh: '折叠详情',
    jp: '折叠详情',
    en: 'Fold Details',
  },
  showDetails: {
    zh: '详细信息',
    jp: '详细信息',
    en: 'Show Details',
  },
  node: {
    zh: '节点',
    jp: '节点',
    en: 'Node',
  },
  view: {
    zh: '点击查看',
    jp: '点击查看',
    en: 'View',
  },
  hide: {
    zh: '点击收起',
    jp: '点击收起',
    en: 'Hide',
  },
  detailedMetrics: {
    zh: '详细统计',
    jp: '详细统计',
    en: 'Detailed metrics',
  },
  actionMetricsTips: {
    zh: '节点上的动作统计信息',
    jp: '节点上的动作统计信息',
    en: 'Metrics on nodes',
  },
  detailedStatus: {
    zh: '详细状态',
    jp: '详细状态',
    en: 'Detailed status',
  },
  resourceStatusTips: {
    zh: '节点上的资源状态信息',
    jp: '节点上的资源状态信息',
    en: 'Resource status on nodes',
  },
  status: {
    zh: '状态',
    jp: '状态',
    en: 'Status',
  },
  ruleEnabled: {
    zh: '已启动',
    jp: '已启动',
    en: 'Enabled',
  },
  ruleDisabled: {
    zh: '已停止',
    jp: '已停止',
    en: 'Disabled',
  },
  resourceStatus: {
    zh: '资源状态',
    jp: '资源状态',
    en: 'Status',
  },
  deleteResource: {
    zh: '此操作将停用并删除该资源,确认继续?',
    jp: '此操作将停用并删除该资源,确认继续?',
    en: 'This operation will disable and delete the resource',
  },
  createResources: {
    zh: '创建资源',
    jp: '创建资源',
    en: 'Create',
  },
  resourceTypes: {
    zh: '资源类型',
    jp: '资源类型',
    en: 'Types',
  },
  resourceName: {
    zh: '资源名称',
    jp: '资源名称',
    en: 'Resource Name',
  },
  testConnection: {
    zh: '测试连接',
    jp: '测试连接',
    en: 'Test',
  },
  remark: {
    zh: '备注',
    jp: '备注',
    en: 'Remark',
  },
  pleaseEnter: {
    zh: '请输入',
    jp: '请输入',
    en: 'Please enter',
  },
  pleaseChoose: {
    zh: '请选择',
    jp: '请选择',
    en: 'Please select',
  },
  resourceAvailable: {
    zh: '资源可用',
    jp: '资源可用',
    en: 'available',
  },
  actionType: {
    zh: '动作类型',
    jp: '动作类型',
    en: 'Action Type',
  },
  edit: {
    zh: '编辑',
    jp: '编辑',
    en: 'Edit',
  },
  delete: {
    zh: '删除',
    jp: '删除',
    en: 'Delete',
  },
  remove: {
    zh: '移除',
    jp: '移除',
    en: 'Remove',
  },
  success: {
    zh: '成功',
    jp: '成功',
    en: 'Success',
  },
  fail: {
    zh: '失败',
    jp: '失败',
    en: 'Fail',
  },
  addAction: {
    zh: '添加动作',
    jp: '添加动作',
    en: 'Add action',
  },
  addActions: {
    zh: '新增动作',
    jp: '新增动作',
    en: 'Add Action',
  },
  editActions: {
    zh: '编辑动作',
    jp: '编辑动作',
    en: 'Edit Action',
  },
  useOfResources: {
    zh: '使用资源',
    jp: '使用资源',
    en: 'Use of resources',
  },
  createNew: {
    zh: '新建',
    jp: '新建',
    en: 'Create',
  },
  all: {
    zh: '全部',
    jp: '全部',
    en: 'All',
  },
  totalResources: {
    zh: '全部资源',
    jp: '全部资源',
    en: 'Resources',
  },
  resources: {
    zh: '资源',
    jp: '资源',
    en: 'Resources',
  },
  state: {
    zh: '状态',
    jp: '状态',
    en: 'State',
  },
  reconnect: {
    zh: '重连',
    jp: '重连',
    en: 'Reconnect',
  },
  successfulDeletion: {
    zh: '删除成功',
    jp: '删除成功',
    en: 'Successful deletion',
  },
  details: {
    zh: '详情',
    jp: '详情',
    en: 'Details',
  },
  basicInfo: {
    zh: '基本信息',
    jp: '基本信息',
    en: 'Basic Info',
  },
  available: {
    zh: '可用',
    jp: '可用',
    en: 'available',
  },
  notAvailable: {
    zh: '不可用',
    jp: '不可用',
    en: 'Not available',
  },
  instructions: {
    zh: '资源说明',
    jp: '资源说明',
    en: 'Instructions',
  },
  configuration: {
    zh: '配置信息',
    jp: '配置信息',
    en: 'Configuration',
  },
  ruleEngine: {
    zh: '规则引擎',
    jp: '规则引擎',
    en: 'Rule Engine',
  },
  createRules: {
    zh: '创建规则',
    jp: '创建规则',
    en: 'Create Rules',
  },
  condition: {
    zh: '条件',
    jp: '条件',
    en: 'Condition',
  },
  definingRuleConditionsAndDataProcessing: {
    zh: '使用 SQL 设定规则，对消息数据筛选、编解码、变型，灵活地将处理后的数据无缝转发至数据库、流处理、API 网关等数据目的地',
    jp: '使用 SQL 设定规则，对消息数据筛选、编解码、变型，灵活地将处理后的数据无缝转发至数据库、流处理、API 网关等数据目的地',
    en: 'Use SQL to set rules to filter, encode, decode, and modify message data, and seamlessly forward processed data to data destinations such as databases, stream processing, and API gateways',
  },
  triggerEvent: {
    zh: '触发事件',
    jp: '触发事件',
    en: 'Trigger Event',
  },
  topic: {
    zh: '主题',
    jp: '主题',
    en: 'Topic',
  },
  selectFiled: {
    zh: '查询字段',
    jp: '查询字段',
    en: 'Select field',
  },
  sqlInput: {
    zh: 'SQL 输入',
    jp: 'SQL 输入',
    en: 'SQL',
  },
  availableField: {
    zh: '可用字段',
    jp: '可用字段',
    en: 'Available field',
  },
  selectConditions: {
    zh: '筛选条件',
    jp: '筛选条件',
    en: 'Conditions',
  },
  sqlTest: {
    zh: 'SQL 测试',
    jp: 'SQL 测试',
    en: 'SQL Test',
  },
  inputMetadata: {
    zh: '输入元数据进行 SQL 匹配测试',
    jp: '输入元数据进行 SQL 匹配测试',
    en: 'Input metadata for SQL test',
  },
  testOutput: {
    zh: '测试输出',
    jp: '测试输出',
    en: 'Result',
  },
  currentEventAvailableField: {
    zh: '当前事件可用字段',
    jp: '当前事件可用字段',
    en: 'Current Event Available Field',
  },
  upgradeTip: {
    zh: '升级提示',
    jp: '升级提示',
    en: 'Upgrade Tip',
  },
  jsonDecodeUpgrade: {
    zh: '当前版本取消对 payload 的自动 JSON 解码，选择 payload 中的字段请使用 json_decode 解码，如 SQL 示例。',
    jp: '当前版本取消对 payload 的自动 JSON 解码，选择 payload 中的字段请使用 json_decode 解码，如 SQL 示例。',
    en: 'The current version cancels the automatic JSON decoding of the payload. Select the field in the payload and use json_decode to decode it, as in the SQL example.',
  },
  exampleSql: {
    zh: '规则 SQL 示例',
    jp: '规则 SQL 示例',
    en: 'SQL Example',
  },
  responseAction: {
    zh: '响应动作',
    jp: '响应动作',
    en: 'Action',
  },
  processingMessagesForHitRules: {
    zh: '处理命中规则的消息',
    jp: '处理命中规则的消息',
    en: 'Processing messages for hit rules',
  },
  pleaseSelectTheTriggerEvent: {
    zh: '请选择触发事件',
    jp: '请选择触发事件',
    en: 'Please select the trigger event',
  },
  pleaseEnterTheTopic: {
    zh: '请输入主题',
    jp: '请输入主题',
    en: 'Please enter the topic',
  },
  pleaseEnterTheSelectField: {
    zh: '请输入查询字段',
    jp: '请输入查询字段',
    en: 'Please enter the select field',
  },
  pleaseEnterTheSQL: {
    zh: '请输入 SQL 语句',
    jp: '请输入 SQL 语句',
    en: 'Please enter the SQL field',
  },
  nullCharacterError: {
    zh: '空字符错误',
    jp: '空字符错误',
    en: 'Null character error',
  },
  fieldFillingError: {
    zh: '字段填写错误',
    jp: '字段填写错误',
    en: 'field required',
  },
  pleaseAddAResponseAction: {
    zh: '请添加响应动作',
    jp: '请添加响应动作',
    en: 'Please add a action',
  },
  createSuccess: {
    zh: '创建成功',
    jp: '创建成功',
    en: 'Create Success',
  },
  editSuccess: {
    zh: '编辑成功',
    jp: '编辑成功',
    en: 'Edit Success',
  },
  resultIsEmpty: {
    zh: '输出为空',
    jp: '输出为空',
    en: 'Result is empty',
  },
  checkForErrors: {
    zh: '请检查 SQL 语句',
    jp: '请检查 SQL 语句',
    en: 'SQL is error',
  },
  quickStart: {
    zh: '快速开始',
    jp: '快速开始',
    en: 'Quick Start',
  },
  productDocumentation: {
    zh: '产品文档',
    jp: '产品文档',
    en: 'Document',
  },
  monitor: {
    zh: '监控',
    jp: '监控',
    en: 'Monitor',
  },
  describe: {
    zh: '描述',
    jp: '描述',
    en: 'Description',
  },
  ruleMetrics: {
    zh: '规则统计',
    jp: '规则统计',
    en: 'Metrics',
  },
  matched: {
    zh: '命中次数',
    jp: '命中次数',
    en: 'matched',
  },
  second: {
    zh: '秒',
    jp: '秒',
    en: 'second',
  },
  times: {
    zh: '次',
    jp: '次',
    en: 'matched',
  },
  currentSpeed: {
    zh: '当前速度',
    jp: '当前速度',
    en: 'Current Speed',
  },
  maximumSpeed: {
    zh: '最大执行速度',
    jp: '最大执行速度',
    en: 'Maximum speed',
  },
  last5MinutesSpeed: {
    zh: '最近5分钟执行速度',
    jp: '最近5分钟执行速度',
    en: 'Last 5 minutes speed',
  },
  actionMetrics: {
    zh: '动作统计',
    jp: '动作统计',
    en: 'Action metrics',
  },
  deleteRuleConfirm: {
    zh: '此操作将停用并删除该规则,确认继续？',
    jp: '此操作将停用并删除该规则,确认继续？',
    en: 'This operation will disable and delete the rule',
  },
  runningMetrics: {
    zh: '运行统计',
    jp: '运行统计',
    en: 'Metrics',
  },
  numberOfHits: {
    zh: '命中次数',
    jp: '命中次数',
    en: 'Hits',
  },
  numberOfRulesExecutedAfterEnabling: {
    zh: '规则启用后的执行次数',
    jp: '规则启用后的执行次数',
    en: 'Number of rules hits after enabling',
  },
  averageExecutionSpeedInTheLast5Minutes: {
    zh: '最近5分钟平均执行速度',
    jp: '最近5分钟平均执行速度',
    en: 'Average speed in the last 5 minutes',
  },
  rule: {
    zh: '规则',
    jp: '规则',
    en: 'Rule',
  },
  messageProcessingForHitRules: {
    zh: '命中规则的消息处理方式',
    jp: '命中规则的消息处理方式',
    en: 'Message processing for rule data',
  },
  clientid_doc: {
    en: 'Current MQTT clientid',
    zh: '当前客户端 MQTT clientid',
    jp: '当前客户端 MQTT clientid',
  },
  username_doc: {
    en: 'Current MQTT username',
    zh: '当前客户端 MQTT username',
    jp: '当前客户端 MQTT username',
  },
  event_doc: {
    en: 'Trigger event name',
    zh: '触发事件名称',
    jp: '触发事件名称',
  },
  id_doc: {
    en: 'MQTT message id',
    zh: 'MQTT message id',
    jp: 'MQTT message id',
  },
  payload_doc: {
    en: 'The payload, if in JSON format, will be automatically decoded, and the object information will be obtained by using payload.x in SQL',
    zh: '消息内容, 如果是 JSON 格式将自动解码, 在 SQL 中使用 payload.x 获取对象信息',
    jp: '消息内容, 如果是 JSON 格式将自动解码, 在 SQL 中使用 payload.x 获取对象信息',
  },
  peername_doc: {
    en: 'Client peer host',
    zh: '客户端网络地址',
    jp: '客户端网络地址',
  },
  qos_doc: {
    en: 'Enumeration of message QoS 0,1,2',
    zh: '消息 QoS 0,1,2 中枚举',
    jp: '消息 QoS 0,1,2 中枚举',
  },
  timestamp_doc: {
    en: 'Timestamp(millisecond)',
    zh: '当前毫秒级时间戳',
    jp: '当前毫秒级时间戳',
  },
  topic_doc: {
    en: 'Currently MQTT topic can be filtered by wildcards in SQL.  When multiple topics are included in subscribe and unsubscribe, only the first one will be obtained here. To obtain all topics, please use topic_filters',
    zh: '当前 MQTT 主题, SQL 中可以使用通配符进行筛选.\nSubscribe 与 Unsubscribe 请求中包含多个主题时, 这里只会获取到第一个, 如需获取全部请使用 topic_filters',
    jp: '当前 MQTT 主题, SQL 中可以使用通配符进行筛选.\nSubscribe 与 Unsubscribe 请求中包含多个主题时, 这里只会获取到第一个, 如需获取全部请使用 topic_filters',
  },
  node_doc: {
    en: 'Node name of the trigger event',
    zh: '触发事件的节点名称',
    jp: '触发事件的节点名称',
  },
  message_delivered: {
    en: 'message delivered',
    zh: '消息投递',
    jp: '消息投递',
  },
  message_acked: {
    en: 'message acked',
    zh: '消息应答',
    jp: '消息应答',
  },
  message_dropped: {
    en: 'message dropped',
    zh: '消息丢弃',
    jp: '消息丢弃',
  },
  client_connected: {
    en: 'client connected',
    zh: '连接建立',
    jp: '连接建立',
  },
  client_disconnected: {
    en: 'client disconnected',
    zh: '连接断开',
    jp: '连接断开',
  },
  session_subscribed: {
    en: 'session subscribed',
    zh: '会话订阅',
    jp: '会话订阅',
  },
  session_unsubscribed: {
    en: 'session unsubscribed',
    zh: '会话取消订阅',
    jp: '会话取消订阅',
  },
  parse_confirm: {
    en: 'Detected incompatible old SQL syntax, confirm to automatically convert it to the new version?',
    zh: '检测到不兼容的旧版 SQL 语法，是否自动转换为新版语法？',
    jp: '检测到不兼容的旧版 SQL 语法，是否自动转换为新版语法？',
  },
  fallbackAction: {
    en: 'Fallback Actions',
    zh: '失败备选动作',
    jp: '失败备选动作',
  },
  fallbackActionCreate: {
    en: 'Set the action that will be performed when an action execution error occurs (optional)',
    zh: '设置发生动作执行错误时将执行的动作（可选）',
    jp: '设置发生动作执行错误时将执行的动作（可选）',
  },
  fallbackActionTip: {
    en: 'Action to be performed when the action execution error',
    zh: '动作执行错误时将执行的操作',
    jp: '动作执行错误时将执行的操作',
  },
}
