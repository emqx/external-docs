export default {
  __not: [
    '应用为调用',//呼び出し
    '认证凭证，通过',//認証が成功しました
    '可查询',//検索可能
    '调整',//調整
    '集群信息，对设备进行管理操作',//クラスター情報、設備を管理します
    '请求一组数据',//データセットをリクエストする
    '密码长度为三到三十二位',//パスワードの長さは3～32文字です
    '密码长度为三到三十二位',//パスワードの長さは3～32文字です
    '取消修改密码，删除密码数据',//パスワードの変更を破棄して、データを削除します
  ],
  confirmDelete: {
    zh: '确认删除应用?',
    jp: '削除してもよろしいですか?',
    en: 'Confirm delete?',
  },
  applicationIsCalled: {
    zh: '应用为调用 REST API 认证凭证，通过 REST API 可查询、调整 EMQ X 集群信息，对设备进行管理操作',
    jp: 'アプリケーションはRESTAPIの呼び出しで認証します、EMQ X クラスターの情報を検索や変更して、設備を管理することができます',
    en: 'In order to invoke the certificate of REST API, the application can query and adjust EMQ X cluster information through REST API, and manage and operate the equipment',
  },
  appId: {
    zh: '应用 ID',
    jp: 'アプリ ID',
    en: 'AppID',
  },
  application: {
    zh: '应用',
    jp: 'アプリケーション',
    en: 'Application',
  },
  productDocumentation: {
    zh: '产品文档',
    jp: 'ドキュメント',
    en: 'Document',
  },
  appName: {
    zh: '应用名称',
    jp: 'アプリ名',
    en: 'App Name',
  },
  expireAt: {
    zh: '到期时间',
    jp: '失効日時',
    en: 'Expire At',
  },
  remark: {
    zh: '备注',
    jp: '備考',
    en: 'Remark',
  },
  isEnabled: {
    zh: '是否启用',
    jp: '有効フラグ',
    en: 'Is Enabled',
  },
  enabled: {
    zh: '启用',
    jp: '有効',
    en: 'Enabled',
  },
  edit: {
    zh: '编辑',
    jp: '編集',
    en: 'Edit',
  },
  delete: {
    zh: '删除',
    jp: '削除',
    en: 'Delete',
  },
  editApp: {
    zh: '编辑应用',
    jp: 'アプリ編集',
    en: 'Edit',
  },
  createApp: {
    zh: '创建应用',
    jp: 'アプリ新規',
    en: 'Create App',
  },
  secret: {
    zh: '密钥',
    jp: 'シークレット',
    en: 'Secret',
  },
  disabled: {
    zh: '不启用',
    jp: '無効',
    en: 'Disabled',
  },
  pleaseEnterAppName: {
    zh: '请输入应用名称',
    jp: 'アプリ名称を入力してください',
    en: 'Please enter the app name.',
  },
  pleaseEnterTheAppId: {
    zh: '请输入应用 ID',
    jp: 'アプリIDを入力してください',
    en: 'Please enter the appid',
  },
  pleaseChoose: {
    zh: '请选择',
    jp: '選択してください',
    en: 'Please select',
  },
  pleaseEnter: {
    zh: '请输入',
    jp: '入力してください',
    en: 'Please enter',
  },
  neverExpire: {
    zh: '永不过期',
    jp: '期限なし',
    en: 'Never expire',
  },
  editorialSuccess: {
    zh: '编辑成功',
    jp: '更新しました',
    en: 'Edit Success',
  },
  successfulAppCreation: {
    zh: '创建应用成功',
    jp: 'アプリが成功に作成しました',
    en: 'Successful app creation',
  },
  determineToDeleteTheApp: {
    zh: '确定删除该应用',
    jp: 'アプリを削除することを確認',
    en: 'Determine to delete the app',
  },
  successfulDeletion: {
    zh: '删除成功',
    jp: '削除が成功しました',
    en: 'Successful deletion',
  },
  cancelConfirm: {
    zh: '是否取消本次修改？',
    jp: '今回の編集内容を破棄しますか',
    en: 'You have unsaved changes, confirm cancel?',
  },
  confirm: {
    zh: '确认信息',
    jp: '確認',
    en: 'Confirm',
  },
  no: {
    zh: '不',
    jp: 'いいえ',
    en: 'NO',
  },
  user: {
    zh: '用户',
    jp: 'ユーザー',
    en: 'Users',
  },
  manageDashboardUsers: {
    zh: 'Dashboard 登录管理用户',
    jp: 'ダッシュボードのユーザー',
    en: 'Manage Dashboard users',
  },
  userName: {
    zh: '用户名',
    jp: 'ユーザー名',
    en: 'Username',
  },
  editorUser: {
    zh: '编辑用户',
    jp: 'ユーザー編集',
    en: 'Edit',
  },
  creatingUser: {
    zh: '创建用户',
    jp: 'ユーザー新規',
    en: 'Create',
  },
  oldPassword: {
    zh: '旧密码',
    jp: '古いパスワード',
    en: 'Old password',
  },
  password: {
    zh: '密码',
    jp: 'パスワード',
    en: 'Password',
  },
  newPassword: {
    zh: '新密码',
    jp: '新しいパスワード',
    en: 'New password',
  },
  confirmPassword: {
    zh: '确认新密码',
    jp: 'パスワード確認用',
    en: 'Confirm password',
  },
  dontChangePassword: {
    zh: '不修改密码',
    jp: 'パスワード変更なし',
    en: 'Dot change password',
  },
  changePassword: {
    zh: '修改密码',
    jp: 'パスワード変更',
    en: 'Change Password',
  },
  pleaseEnterYourPasswordAgain: {
    zh: '请再次输入密码',
    jp: 'パスワードをもう一度入力してください',
    en: 'Please enter your password again.',
  },
  confirmNotMatch: {
    zh: '两次输入密码不一致',
    jp: '新パスワードと再入力パスワードが一致しません',
    en: 'Two inputs don\'t match!',
  },
  twoInconsistenciesInPasswordInput: {
    zh: '两次输入密码不一致',
    jp: '新パスワードと再入力パスワードが一致しません',
    en: 'Two inconsistencies in password input',
  },
  enterOneUserName: {
    zh: '请输入用户名',
    jp: 'ユーザー名を入力してください',
    en: 'Please enter username',
  },
  pleaseEnterNotes: {
    zh: '请输入备注',
    jp: '備考を入力してください',
    en: 'Please enter notes',
  },
  pleaseEnterPassword: {
    zh: '请输入密码',
    jp: '古いパスワードを入力してください',
    en: 'Please enter password',
  },
  pleaseEnterNewPassword: {
    zh: '请输入新密码',
    jp: '新しいパスワードを入力してください',
    en: 'Please enter new password',
  },
  pleaseEnterAConfirmationPassword: {
    zh: '请输入确认密码',
    jp: '新しいパスワードと同じ値を再度入力してください',
    en: 'Please enter confirm password.',
  },
  createUserSuccess: {
    zh: '创建用户成功',
    jp: 'ユーザーが成功に作成しました',
    en: 'Create User Success',
  },
  determineToDeleteTheUser: {
    zh: '确定删除该用户',
    jp: 'ユーザーが成功に削除しました',
    en: 'Confirm to delete the user',
  },
  passwordLength: {
    zh: '密码长度为 3~32 位',
    jp: 'パスワードの長さは3～32文字です',
    en: 'Password length is 3~32',
  },
  confirmDeleteUser: {
    zh: '确认删除该用户?',
    jp: 'ユーザーを削除してもよろしいですか?',
    en: 'Confirm delete?',
  },
  blacklistTips: {
    zh: '通过黑名单禁止客户端建立连接，该功能适用于管理少量客户端，客户端认证请使用',
    jp: 'クライアントがブラックリストのエントリに一致する場合、接続が拒否されます。この機能は少数クライアントの認証をお勧めします',
    en: 'The blacklist prohibits clients from establishing connections. This function is suitable for managing a small number of clients. For client authentication, view',
  },
  authDocs: {
    zh: '认证文档',
    jp: '認証ドキュメント',
    en: 'authentication document',
  },
  blacklist: {
    zh: '黑名单',
    jp: 'ブラックリスト',
    en: 'Blacklist',
  },
  createBlacklist: {
    zh: '创建黑名单',
    jp: 'ブラックリスト作成',
    en: 'Create',
  },
  determineToDeleteTheBlacklist: {
    zh: '确定删除该黑名单',
    jp: 'ブラックリストを削除してもよろしいですか',
    en: 'Determine to delete the blacklist',
  },
  createBlacklistSuccess: {
    zh: '创建黑名单成功',
    jp: 'ブラックリストが成功に更新しました',
    en: 'Create Blacklist Success',
  },
  who: {
    zh: '禁用值',
    jp: '値',
    en: 'Value',
  },
  as: {
    zh: '禁用属性',
    jp: 'プロパティ',
    en: 'Property',
  },
  reason: {
    zh: '原因',
    jp: '原因',
    en: 'Reason',
  },
  desc: {
    zh: '描述',
    jp: '詳細',
    en: 'Description',
  },
  until: {
    zh: '到期时间',
    jp: '失効日時',
    en: 'Until',
  },
  enterWho: {
    zh: '请输入禁用值',
    jp: '値を入力してください',
    en: 'Please enter value',
  },
  enterAs: {
    zh: '请选择禁用属性',
    jp: 'プロパティを入力してください',
    en: 'Please select property',
  },
}
